...
export class CustomersService {
  ...
  async update(id: string, updateCustomerDto: UpdateCustomerDto): Promise<UpdateResult> {
    try {
      const customer = await this.customerRepository.update( {id: id, isActive: true}, updateCustomerDto);
  ...
  async remove(id: string) {
    try {
      const customer = await this.customerRepository.update( {id: id, isActive: true}, {isActive: false});
      if (customer.affected === 0){
        throw new ManagerError({
          type: 'NOT_FOUND',
          message: 'Customer not found',
